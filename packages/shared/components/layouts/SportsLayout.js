...
const SportsLayout = ({
  ...
  const handleBetSelect = (betId) => {
    setSelectedBets(prev => {
      ...
      setSelectAll(newSelection.length === betData.length && betData.length > 0);
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      // Select all
      const allBetIds = betData.map(bet => bet.id);
  ...
  const handleDeleteAll = async () => {
    if (selectedBets.length === 0) {
      // Show toast message - no bets selected
      Notify('Please select at least one bet to delete', null, null, 'danger');
      return;
    }

    const confirmed = await showConfirm({
      title: "Delete Selected Bets",
      message: `Are you sure you want to delete ${selectedBets.length} selected bet(s)? This action cannot be undone.`,
      confirmText: "Delete",
      cancelText: "Cancel",
      type: "danger",
      onConfirm: async () => {
        try {
          ...
          // Call the API to delete bets
          const response = await adminApi(`${ADMIN_BASE_PATH}/delete-multiple-bets`, 'POST', {
            bet_ids: JSON.stringify(selectedBets),
            is_deleted: 1 // 1 for delete
          });

          if (response.success) {
            Notify(response.message || `Successfully deleted ${response.data?.processed || selectedBets.length} bet(s)`, null, null, 'success');
            ...
            Notify(response.message || 'Failed to delete bets', null, null, 'danger');
          }
        } catch (error) {
          console.error('Error deleting bets:', error);
          Notify('Failed to delete bets. Please try again.', null, null, 'danger');
        } finally {
          setLoadingBets(false);
        }
      }
  ...
  // Handle Rollback All action
  const handleRollbackAll = async () => {
    if (selectedBets.length === 0) {
      // Show toast message - no bets selected
      Notify('Please select at least one bet to rollback', null, null, 'danger');
      return;
    }

    const confirmed = await showConfirm({
      title: "Rollback Selected Bets",
      message: `Are you sure you want to rollback ${selectedBets.length} selected bet(s)? This action cannot be undone.`,
      ...
      onConfirm: async () => {
        try {
          ...
          // Call the API to restore bets
          const response = await adminApi(`${ADMIN_BASE_PATH}/delete-multiple-bets`, 'POST', {
            bet_ids: JSON.stringify(selectedBets),
            is_deleted: 0 // 0 for restore/rollback
          });

          if (response.success) {
            Notify(response.message || `Successfully restored ${response.data?.processed || selectedBets.length} bet(s)`, null, null, 'success');
            
            // Clear selection after successful rollback
            setSelectedBets([]);
            setSelectAll(false);
            ...
            Notify(response.message || 'Failed to rollback bets', null, null, 'danger');
          }
        } catch (error) {
          console.error('Error rolling back bets:', error);
          Notify('Failed to rollback bets. Please try again.', null, null, 'danger');
        } finally {
          setLoadingBets(false);
        }
      }
