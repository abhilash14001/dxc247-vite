...
setBetOddValue(0);
    ...
    if (!teamNames.current['ODDS']) {
        teamNames.current['ODDS'] = []; // Initialize 'ODDS' as an empty array
    }
    ...
    const updateOdds = (backlay, odds, tot) => {
        ...
        <div className="game-market market-4">
            ...
            >
                                    {mainValue?.['section']?.map((oddsArr, key) => {
                                        ...
                                        if (!teamNames.current['ODDS'].includes(teamName)) {
                                            teamNames.current['ODDS'].push(teamName);
                                        }
                                        ...
                                        const backOdds = (oddsArr.back.length === 1
                                            ? backArr
                                            : backArr.slice().sort((a, b) => b.tno -a.tno)
                                        ).map((back, b_key) => {
                                            ...
                                            if (oddsChange.current[`oddsback${key}${b_key}`] === undefined) {
                                                ...
                                                updateOdds(`oddsback${key}${b_key}`, back.odds, tot);
                                            }
                                            ...
                                            return (
                                                <div
                                                    className={`market-odd-box ${backClass} ${tot !== 0 && oddsChange.current[`oddsback${key}${b_key}`] !== back.odds && oddsChange.current[`oddsback${key}${b_key}`] !== undefined ? 'blink' : ''}`}
                                                    onClick={backFunction[back.tno]}
                                                    key={back.tno}
                                                >
                                                    <span className="market-odd">{ tot === 0 ? '-' : back.odds || "-"}</span>
                                        ...
                                        const layOdds = layArr.slice().sort((a, b) => a.tno - b.tno).map((lay, l_key) => {
                                            ...
                                            if (oddsChange.current[`oddslay${key}${l_key}`] === undefined) {
                                                ...
                                                updateOdds(`oddslay${key}${l_key}`, lay.odds, tot);
                                            }
                                            ...
                                            return (
                                                <div
                                                    className={`market-odd-box ${layClass} ${tot !== 0 && oddsChange.current[`oddslay${key}${l_key}`] !== lay.odds && oddsChange.current[`oddslay${key}${l_key}`] !== undefined ? 'blink' : ''}`}
                                                    onClick={layFunction[lay.tno]}
                                                    key={l_key}
                                                >
                                                    <span className="market-odd">{ tot === 0 ? '-' : lay.odds || "-"}</span>
                                            ...
                                            >
                                                <div className="market-nation-detail">
                                                    ...
                                                    <div className="market-nation-book">
                                                        ...
                                                        <span
                                                            className={`market-live-book d-none d-xl-block ${placingBets?.['ODDS']?.[teamName] < 0 ? 'text-danger' : 'text-success'}`}>
                                                            {placingBets?.['ODDS']?.[teamName] ?? ''}
                                                        </span>
                                                    </div>
                                                </div>
                                                {backOdds}
