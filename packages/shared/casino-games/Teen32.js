...
const Teen32 = () => {
    ...
    const [playerStatuses, setPlayerStatuses] = useState({"Player A": 'suspended-row', "Player B": 'suspended-row'});
        ...
        if (isMounted) {
            ...
            if (data.card) {
                ...
                setCards({
                    ...
                    playerB: cardArray.filter((_, index) => index % 2 !== 0),
                });
    ...
    useEffect(() => {
        ...
        if (data?.sub && sportList?.id) {
            getExByTeamNameForCasino(sportList.id, data.mid, 'Player A', match_id).then(res => {
                if (isMounted) setPlayerA(res.data)
            });

            getExByTeamNameForCasino(sportList.id, data.mid, 'Player B', match_id).then(res => {
                if (isMounted) setPlayerB(res.data)
            });
        }
                    ...
                    <div className="casino-table-box">
                        {
                            ["Player A", "Player B"].map((playerName, i) => ([
                                    ...
                                    />,
                                    i === 0 && <div key={'divider'} className="casino-table-box-divider"></div>
                                ]
    ...
    <div className="casino-table-left-box">
        ...
        <div className={`casino-table-body`}>
            <div className={`casino-table-row ${playerStatus}`}>
                ...
                <div className="casino-odds-box back" onClick={() => openPopup('back', playerName, playerBack)}>
                    <span className="casino-odds">{playerBack}</span>
                </div>
                <div className="casino-odds-box lay" onClick={() => openPopup('lay', playerName, playerLay)}>
