...
<div>PAYOUT: 12.0</div>
    ...
    return (
        <CasinoLayout raceClass="lucky7a" ruleDescription={ruleDescription} hideLoading={hideLoading} isBack={backOrLay} teamname={teamname} handleStakeChange={casinoBetDataNew} odds={odds}
                    ...
                    {Object.entries(totalPlayers[0].ODDS).map(([index, value], i) => (
                        <>
                            <div className={`lucky7${index.toLowerCase().replace(' ', '').replace('card', '')}`} key={i}>
                                ...
                                <div className="casino-nation-book text-center">{getExByColor(value.amounts)}</div>
                            </div>
                            {i === 0 && (
