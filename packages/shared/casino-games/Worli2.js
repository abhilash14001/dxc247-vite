...
const Worli2 = () => {
    ...
    const updateAmount = async () => {
        ...
        setTotalPlayers((prevState) => {
            ...
            // Update amounts for SINGLE numbers
            if (prev.SINGLE && prev.SINGLE.name) {
                prev.SINGLE.amounts = {};
                prev.SINGLE.name.forEach((teamName, index) => {
                    const foundAmount = amountData.data?.find((item) => item.team_name === teamName);
                    prev.SINGLE.amounts[teamName] = foundAmount?.total_amount || '';
                });
            }

            // Update amounts for LINE bets
            if (prev.LINE && prev.LINE.name) {
                prev.LINE.amounts = {};
                prev.LINE.name.forEach((teamName) => {
                    const foundAmount = amountData.data?.find((item) => item.team_name === teamName);
                    prev.LINE.amounts[teamName] = foundAmount?.total_amount || '';
                });
            }

            // Update amounts for EVEN_ODD bets
            if (prev.EVEN_ODD && prev.EVEN_ODD.name) {
                prev.EVEN_ODD.amounts = {};
                prev.EVEN_ODD.name.forEach((teamName) => {
                    const foundAmount = amountData.data?.find((item) => item.team_name === teamName);
                    prev.EVEN_ODD.amounts[teamName] = foundAmount?.total_amount || '';
                });
            }
                            ...
                            <div className="worli-right">
                                ...
                                <div className="worli-box-row">
                                    <div 
                                        ...
                                        <div className="casino- text-danger">
                                            {getExByColor(totalPlayers['LINE'].amounts?.['Line 1 Single'] || '')}
                                        </div>
                                    </div>
                                    <div 
                                        ...
                                        <div className="casino- text-danger">
                                            {getExByColor(totalPlayers['EVEN_ODD'].amounts?.['Odd'] || '')}
                                        </div>
                                    </div>
                                </div>
                                <div className="worli-box-row">
                                    <div 
                                        ...
                                        <div className="casino- text-danger">
                                            {getExByColor(totalPlayers['LINE'].amounts?.['Line 2 Single'] || '')}
                                        </div>
                                    </div>
                                    <div 
                                        ...
                                        <div className="casino- text-danger">
                                            {getExByColor(totalPlayers['EVEN_ODD'].amounts?.['Even'] || '')}
                                        </div>
                                    </div>
                                </div>
                            </div>
